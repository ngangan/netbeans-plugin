 ...
 * @author Pavel Benes
 */
public final class FXDElementOutline {
    ...
    private final FXDElement m_fxdElement;
    ...
    protected static void drawOutline( Graphics g, int xOff, int yOff, float [][] coords) {
        for (int i = 0; i < 4; i++) {
            int j = (i+1) % 4;
    ...
    private synchronized void checkObject() {
        short changeTicker = m_fxdElement.getController().getScreenChangeTicker();
        if ( changeTicker != m_tickerCopy) {
            rect2poly( m_fxdElement.getBounds(), m_coords);
            m_tickerCopy = changeTicker;
        }
        //short ticker = m_fxdElement.getScreenManager().getChangeTicker();
        
        /*
        if (ticker != m_tickerCopy) {
            transformRectangle(m_fxdElement.getSafeBBox(),
                    (Transform) m_fxdElement.getSVGElement().getScreenCTM(), m_coords);
            m_tickerCopy = ticker;
        } */       
    }
