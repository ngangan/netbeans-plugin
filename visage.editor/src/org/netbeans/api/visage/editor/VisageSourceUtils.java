 ...
 */
public final class VisageSourceUtils {

    public static final String VISAGE_MIME_TYPE = "text/x-visage";  // NOI18N
    ...
    private static final Logger log = Logger.getLogger(VisageSourceUtils.class.getName());

    private VisageSourceUtils() {
    }

    /// Editor tooltips
    /// =====================
    public static String getElementTooltip(VisageTypes types, Element element) {
    ...
    private static String typeToString(VisageTypes types, Type type) {
        if (type == null) {
            return "<null>"; // NOI18N
        }
    ...
    private static String methodToString(VisageTypes types, MethodType mtype) {
        StringBuilder s = new StringBuilder();
        s.append('('); // NOI18N
        if (mtype == null) {
            s.append("???"); // NOI18N
    ...
    /// Navigator
    /// =====================
    public static String executableElementToString(VisageTypes types, ExecutableElement element, boolean isDeprecated, boolean isInherited) {
    ...
    public static String variableElementToString(VisageTypes types, VariableElement element, boolean isDeprecated, boolean isInherited) {
        StringBuilder sb = new StringBuilder();
...
//        if (type instanceof FunctionType) {
    ...
    public static String typeElementToString(VisageTypes types, TypeElement element, boolean isDeprecated, boolean isInherited) {
        StringBuilder sb = new StringBuilder();
        if (isDeprecated) {
            sb.append("<s>"); // NOI18N
        }
    ...
    private static String boundsToString(java.util.List<? extends TypeMirror> bounds, VisageTypes types) {
...
//                    sourceRoot = Index.getSourceRootForClassFolder(url);
    ...
    public static boolean isVisageClass(final Element element, final CompilationInfo compilationInfo) {
        ...
        return compilationInfo.getVisageTypes().isVisageClass((Symbol) element);
    }
    ...
    public static boolean isVisageContext(final Document doc, final int offset) {
        ...
        try {
            if (doc.getLength() == 0 && MIME_TYPE_DIALOG_BINDING.equals(doc.getProperty("mimeType"))) { // NOI18N
                ...
                if (d != null) {
                    return VISAGE_MIME_TYPE.equals(NbEditorUtilities.getMimeType(d));
                }
                FileObject fo = (FileObject) attributes.getValue(path, "dialogBinding.fileObject"); // NOI18N
                return VISAGE_MIME_TYPE.equals(fo.getMIMEType());
            }

            TokenSequence<VisageTokenId> ts = getVisageTokenSequence(TokenHierarchy.get(doc), offset, doc);
            if (ts == null) {
                return false;
            }
    ...
    public static TokenSequence<VisageTokenId> getVisageTokenSequence(final TokenHierarchy hierarchy, final int offset, final Document doc) {
        if (hierarchy != null) {
            TokenSequence<?> ts_ = hierarchy.tokenSequence();
            while (ts_ != null && ts_.isValid() && (offset == 0 || ts_.moveNext())) {
                ts_.move(offset);
                if (ts_.language() == VisageTokenId.language()) {
                    return (TokenSequence<VisageTokenId>) ts_;
                }
