 ...
 * @author Dusan Balek
 */
public class VisageCompletionProvider implements CompletionProvider {
    
    private static final Logger logger = Logger.getLogger(VisageCompletionProvider.class.getName());
    ...
    public int getAutoQueryTypes(JTextComponent component, String typedText) {
        if (".".equals(typedText) || (autoMode && VisageCompletionQuery.isJavaIdentifierPart(typedText))) { // NOI18N
            if (FXSourceUtils.isVisageContext(component, component.getSelectionStart() - 1))
    ...
    public static VisageTreePath getPathElementOfKind(Tree.VisageKind kind, VisageTreePath path) {
        return getPathElementOfKind(EnumSet.of(kind), path);
    }
    
    public static VisageTreePath getPathElementOfKind(EnumSet<Tree.VisageKind> kinds, VisageTreePath path) {
        while (path != null) {
            if (kinds.contains(path.getLeaf().getVisageKind()))
                return path;
...
//                    if (LOGGABLE) log("getVisageTokenSequence returning null (1) for offset " + offset); // NOI18N
//                    return null;
//                }
//                ts_ = ts_.embedded();
//            }
//        }
//        if (LOGGABLE) log("getVisageTokenSequence returning null (2) for offset " + offset); // NOI18N
//        return null;
//    }
    
    public CompletionTask createTask(int type, JTextComponent component) {
        if ((type & COMPLETION_QUERY_TYPE) != 0 || type == TOOLTIP_QUERY_TYPE || type == DOCUMENTATION_QUERY_TYPE)
            return new AsyncCompletionTask(new VisageCompletionQuery(type, component.getSelectionStart(), true), component);
        return null;
    }
    
    static CompletionTask createDocTask(ElementHandle element) {
        VisageCompletionQuery query = new VisageCompletionQuery(DOCUMENTATION_QUERY_TYPE, -1, true);
        query.setElement(element);
        return new AsyncCompletionTask(query, EditorRegistry.lastFocusedComponent());
    }

    public static Set<? extends CompletionItem> query(VisageSource source, int queryType, int offset, int substitutionOffset) throws IOException {
        assert source != null;
        assert (queryType & COMPLETION_QUERY_TYPE) != 0;
        VisageCompletionQuery query = new VisageCompletionQuery(queryType, offset, false);
        source.runUserActionTask(query, false);
        if (offset != substitutionOffset) {
            for (VisageCompletionItem jci : query.results) {
